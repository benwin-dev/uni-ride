...
export async function POST(
  ...
  try {
    ...
    if (doc.offeredByUserIds.includes(userId)) {
      return NextResponse.json(docToRequest(doc));
    }

    doc.offeredByUserIds = [...doc.offeredByUserIds, userId];
    await doc.save();

    return NextResponse.json(docToRequest(doc));
  } catch (e) {
    console.error("POST /api/ride-requests/[id]/offer:", e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to offer ride" },
      { status: 500 }
    );
  }
}
