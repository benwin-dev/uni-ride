...
export async function GET(
  ...
  try {
    ...
    console.error("GET /api/ride-requests/[id]:", e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to fetch request" },
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const allowed = [
      ...
      "date", "time", "note", "seatsNeeded", "maxPrice", "status",
    ];
    ...
    const doc = await RideRequestModel.findByIdAndUpdate(
      id,
      { $set: updates },
      { new: true }
    ).exec();
    ...
    console.error("PATCH /api/ride-requests/[id]:", e);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const doc = await RideRequestModel.findByIdAndDelete(id).exec();
    if (!doc) {
      return NextResponse.json({ error: "Request not found" }, { status: 404 });
    }
    ...
    console.error("DELETE /api/ride-requests/[id]:", e);
    return NextResponse.json(
